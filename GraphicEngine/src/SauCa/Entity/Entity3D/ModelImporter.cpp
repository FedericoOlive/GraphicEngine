...
void ModelImporter::LoadModel(std::string const& path, std::string& directory, std::vector<Mesh>& meshes, std::vector<MeshTexture>& textures_loaded,  bool& IsInvertTextures)
{
	...
	ProcessNode(scene->mRootNode, scene, directory, meshes, textures_loaded, IsInvertTextures);
}

void ModelImporter::ProcessNode(aiNode* node, const aiScene* scene, std::string& directory, std::vector<Mesh>& meshes, std::vector<MeshTexture>& textures_loaded, bool& IsInvertTextures)
{
	...
	for (unsigned int i = 0; i < node->mNumMeshes; i++)
	{
		...
		meshes.push_back(ProcessMesh(mesh, scene, textures_loaded, directory, IsInvertTextures));
	}
	// after we've processed all of the meshes (if any) we then recursively process each of the children nodes
	for (unsigned int i = 0; i < node->mNumChildren; i++)
	{
		ProcessNode(node->mChildren[i], scene, directory, meshes, textures_loaded, IsInvertTextures);
	}
}

Mesh ModelImporter::ProcessMesh(aiMesh* mesh, const aiScene* scene, std::vector<MeshTexture>& textures_loaded, std::string& directory, bool& IsInvertTextures)
{
	// data to fill
